...
function draw_function(cr)
	...
	cairo_select_font_face (cr, "monofur for Powerline for Powerline", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL);
	...
	if battery then
		...
		draw_circle(cr,gap_x_fix,gap_y_fix,r_battery,g_battery,b_battery,transparency_battery,conky_parse('${battery_percent}'))
		draw_battery(cr,gap_x+gap_x_fix, gap_y_fix,start_rect_height,r_battery,g_battery, b_battery,transparency_battery)
		
		for i=1,drives do
			draw_circle(cr,(start_rect_width+gap_x_distance)*i+gap_x_fix,gap_y_fix,drive_colors[i][1], drive_colors[i][2], drive_colors[i][3], drive_colors[i][4],conky_parse('${fs_free_perc ' .. drive_paths[i] .. '}'))
			draw_folder(cr,(start_rect_width+gap_x_distance)*i+gap_x_fix,gap_y_fix,start_rect_height,drive_paths[i],drive_names[i],drive_colors[i][1], drive_colors[i][2], drive_colors[i][3], drive_colors[i][4])
		end

		draw_circle(cr,(start_rect_width+gap_x_distance)*(drives+1)+gap_x_fix,gap_y_fix,r_CPU, g_CPU, b_CPU, transparency_CPU,conky_parse('${cpu cpu0}'))
		draw_cpu(cr,number_of_cpus, (start_rect_width+gap_x_distance)*(drives+1)+gap_x+gap_x_fix,gap_y_fix+gap_y,r_CPU, g_CPU, b_CPU, transparency_CPU)

		draw_circle(cr,(start_rect_width+gap_x_distance)*(drives+2)+gap_x_fix,gap_y_fix,r_RAM, g_RAM, b_RAM, transparency_RAM,tostring(100-tonumber(conky_parse('${memperc}'))))
		draw_ram(cr,(start_rect_width+gap_x_distance)*(drives+2)+gap_x+gap_x_fix,gap_y+gap_y_fix, radius, r_RAM, g_RAM, b_RAM, transparency_RAM)

	else
	
		for i=1,drives do
			draw_circle(cr,(start_rect_width_no_battery)+gap_x_fix,gap_y_fix,drive_colors[i][1], drive_colors[i][2], drive_colors[i][3], drive_colors[i][4],conky_parse('${fs_free_perc ' .. drive_paths[i] .. '}'))
			draw_folder(cr,(start_rect_width_no_battery)+gap_x_fix,gap_y_fix,start_rect_height,drive_paths[i],drive_names[i],drive_colors[i][1], drive_colors[i][2], drive_colors[i][3], drive_colors[i][4])
			start_rect_width_no_battery = start_rect_width_no_battery + gap_x_distance + 108
		end

		draw_circle(cr,(start_rect_width_no_battery)+gap_x_fix,gap_y_fix,r_CPU, g_CPU, b_CPU, transparency_CPU,conky_parse('${cpu cpu0}'))
		draw_cpu(cr,number_of_cpus,(start_rect_width_no_battery)+gap_x+gap_x_fix,gap_y_fix+gap_y,r_CPU, g_CPU, b_CPU, transparency_CPU)
		start_rect_width_no_battery = start_rect_width_no_battery + gap_x_distance + 108

		draw_circle(cr,(start_rect_width_no_battery)+gap_x_fix,gap_y_fix,r_RAM,g_RAM,b_RAM,transparency_RAM,tostring(100-tonumber(conky_parse('${memperc}'))))
		draw_ram(cr,(start_rect_width_no_battery)+gap_x+gap_x_fix,gap_y+gap_y_fix,radius,r_RAM,g_RAM,b_RAM,transparency_RAM)		
